# Khadija Ali
**BS Artificial Intelligence Student** | **Agentic AI & LLM Systems** | **Machine Learning**

I build **real-world AI systems** — from machine learning classifiers to LLM-powered agents and workflow automation — using Python, LangChain, n8n, and Streamlit to solve practical problems across healthcare, governance, and technology domains.
My focus is on creating **end-to-end applications** that go from raw data or a user query to a working, usable output.
...
- **Scholar Router (n8n Workflow)** – [n8n-scholar-router](https://github.com/khadijaa-ali/n8n-scholar-router)
  n8n automation workflow that classifies incoming messages into 4 intent types and routes each to a different LLM chain or agent using conditional Switch/If logic.
  Solved a tricky back-reference expression bug in multi-branch routing — now a reusable fix pattern for any multi-branch n8n workflow.
...
- **SmartPhone Price Predictor** – [SmartPhone-Price-Predictor](https://github.com/khadijaa-ali/SmartPhone-Price-Predictor)
  Dual-model ML app that predicts mobile price category (Budget / Mid-Range / Upper Mid-Range / Premium) using XGBoost Classifier and estimates exact price using Random Forest Regressor.
  Compared 3 classifiers and 3 regressors; selected best models based on Accuracy, F1-Score, ROC-AUC and MAE.
  Deployed as an interactive Streamlit app with confidence bars per class and real-time price estimation.
...
- **Currency Converter App** – [Currency-Converter](https://github.com/khadijaa-ali/Currency-Converter)
  Streamlit web app that converts any currency using live exchange rates from an API.
